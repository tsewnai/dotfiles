...
Item {
  ...
  // Only one screen shows the password field: the first of these that is
  // connected. DP-6 is the center external monitor; eDP-1 the laptop panel.
  // The trailing fallback (first real screen) guarantees the field is never
  // hidden on every screen, which would otherwise lock you out.
  readonly property var preferredInputScreens: ["DP-6", "eDP-1"]
  readonly property string inputScreenName: {
    var screens = Quickshell.screens || []
    var names = []
    for (var i = 0; i < screens.length; i++) {
      var s = screens[i]
      if (s && s.name && s.width > 0 && s.height > 0) names.push(s.name)
    }
    for (var p = 0; p < preferredInputScreens.length; p++) {
      if (names.indexOf(preferredInputScreens[p]) !== -1) return preferredInputScreens[p]
    }
  ...
  WlSessionLock {
    ...
    WlSessionLockSurface {
      ...
      LockView {
        id: lockView
        anchors.fill: parent
        showInputField: root.inputScreenName === "" || (lockSurface.screen && lockSurface.screen.name === root.inputScreenName)
